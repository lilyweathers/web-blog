...
async function loadPosts() {
  ...
  for (const p of data.posts) {
    ...
    const likeCountDisplay = p.likeCount + (p.liked && !likedLocal ? -1 : 0) + (likedLocal ? 1 : 0);
    const div = document.createElement('div');
    div.className = 'post';
    div.dataset.id = p.id;
    div.innerHTML = `
      <h3>${escapeHtml(p.title)}</h3>
      <div class="meta">by <strong>${escapeHtml(p.author || 'Anonymous')}</strong> • created ${fmtDate(p.createdAt)}${p.updatedAt && p.updatedAt !== p.createdAt ? ` • updated ${fmtDate(p.updatedAt)}`:''}</div>
...
function renderComments(comments) {
  if (!comments || comments.length === 0) return `<div class="comment" style="opacity:.7">No comments yet. Be the first one to comment!</div>`;
  return comments
    .sort((a,b) => a.createdAt - b.createdAt)
    .map(c => `<div class="comment"><div class="meta">${escapeHtml(c.author || 'Anonymous')} • ${fmtDate(c.createdAt)}</div>${escapeHtml(c.content)}</div>`)
    .join('');
}
...
async function onPostsClick(e) {
  ...
  if (e.target.matches('button[data-edit]')) {
    ...
    const authorText = (postEl.querySelector('.meta')?.textContent || '').match(/by\s+(.*?)\s+•/i);
    const currentAuthor = authorText ? authorText[1] : 'Anonymous';
    ...
    if (newTitle != null || newContent != null || newAuthor != null) {
      await fetchJSON(`/api/posts/${id}`, { method: 'PUT', body: JSON.stringify({ title: newTitle, content: newContent, author: newAuthor }) });
      await loadPosts();
    }
    return;
  }
  ...
  if (e.target.closest('[data-like]')) {
    ...
    heart.textContent = nextLike ? '♥' : '♡';
    const cur = parseInt(countEl.textContent, 10) || 0;
    countEl.textContent = String(Math.max(0, cur + (nextLike ? 1 : -1)));
    setLocalLiked(id, nextLike);
    // Inform server
    try {
      await fetchJSON(`/api/posts/${id}/like`, { method: 'PUT', body: JSON.stringify({ like: nextLike }) });
    } catch (e) {
      // revert on error
      setLocalLiked(id, currentlyLiked);
      heart.classList.toggle('liked', currentlyLiked);
      heart.textContent = currentlyLiked ? '♥' : '♡';
...
loadPosts().catch(err => {
  const list = document.getElementById('list');
  list.innerHTML = `<div class="post">Failed to load posts: ${err.message}</div>`;
});
